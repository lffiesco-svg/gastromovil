...
function sendIA() {
    ...
    typing.className = 'bg-white p-3 rounded-2xl rounded-bl-sm text-sm text-gray-700 shadow-sm max-w-xs';
    ...
    fetch('/api/chat/', {
        ...
        body: JSON.stringify({ mensaje: text, historial: historialChat })
    })
    .then(res => res.json())
    .then(data => {
        typing.textContent = data.respuesta;
    ...
    .catch(() => {
        typing.textContent = 'Error al conectar con el asistente.';
        messages.scrollTop = messages.scrollHeight;
    });
}
